...
`;l.appendChild(a);l.appendChild(b);e.appendChild(l);const u=document.createElement("div");u.style.cssText="max-width: 720px; margin: 16px auto 0; display: grid; grid-template-columns: 1fr 1fr; gap: 12px;";u.innerHTML=`
...
`;e.appendChild(u);let m=0,g=0;function w(){m=(m+.5)%360;const t=Math.round(m);s.style.background=`hsl(${t}, 70%, 20%)`,s.style.borderColor=`hsl(${t}, 70%, 40%)`,s.textContent=`🎨 Main thread alive — hue: ${t}°`,g=requestAnimationFrame(w)}g=requestAnimationFrame(w);const C=t=>{a.parentNode&&a.remove(),n.style.background="#1a3a1a",n.style.borderColor="#2ea043",n.textContent="✅ Worker ready",console.log("async-dom Svelte action instance ready:",t)},T=t=>{console.error("async-dom error:",t)},L=E(b,{worker:()=>new Worker(new URL("/assets/worker-BlkfJms8.js",import.meta.url),{type:"module"}),onReady:C,onError:T});window.addEventListener("unload",()=>{cancelAnimationFrame(g),L.destroy()});
