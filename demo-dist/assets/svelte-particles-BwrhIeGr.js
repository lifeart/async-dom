...
`;e.appendChild(u);let m=0,g=0;function w(){m=(m+.5)%360;const t=Math.round(m);a.style.background=`hsl(${t}, 70%, 20%)`,a.style.borderColor=`hsl(${t}, 70%, 40%)`,a.textContent=`🎨 Main thread alive — hue: ${t}°`,g=requestAnimationFrame(w)}g=requestAnimationFrame(w);const C=t=>{s.parentNode&&s.remove(),n.style.background="#1a3a1a",n.style.borderColor="#2ea043",n.textContent="✅ Worker ready",console.log("async-dom Svelte action instance ready:",t)},T=t=>{console.error("async-dom error:",t)},L=E(b,{worker:()=>new Worker(new URL("/assets/worker-a4lXtOsj.js",import.meta.url),{type:"module"}),onReady:C,onError:T});window.addEventListener("unload",()=>{cancelAnimationFrame(g),L.destroy()});
