...
var FrameScheduler = class {
	...
	getActionsForFrame() {
		...
		if (queueLen > 1500) return require_ws_transport.CRITICAL_QUEUE_SIZE;
	...
	function renderWarningsTab() {
		...
		if (warnViewMode === "chronological") for (const entry of visible) fragment.appendChild(buildWarnEntryDiv(entry));
			...
			for (const [code, entries] of groups) {
				...
				const desc = require_sync_channel.WarningDescriptions[code];
				if (desc) {
...
function createAsyncDom(config) {
	...
	const debugStats = new require_sync_channel.DebugStats();
	const causalityTracker = new CausalityTracker();
	const workerPerfEntries = /* @__PURE__ */ new Map();
	const MAX_PERF_ENTRIES = 200;
	const mutationCorrelation = new require_sync_channel.MutationEventCorrelation();
	...
	threadManager.onMessage((appId, message) => {
		...
		if (require_sync_channel.isSystemMessage(message) && message.type === "eventTimingResult") {
			const bridge = eventBridges.get(appId);
			if (bridge) bridge.updateTraceWithWorkerTiming(message.listenerId, message.dispatchMs, message.mutationCount);
			return;
		}
		if (require_sync_channel.isSystemMessage(message) && message.type === "perfEntries") {
...
Object.defineProperty(exports, "FrameScheduler", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "createAsyncDom", {
	enumerable: true,
	get: function() {
		return createAsyncDom;
	}
});
Object.defineProperty(exports, "main_thread_exports", {
	enumerable: true,
	get: function() {
		return main_thread_exports;
	}
});
