...
export function importSession(json: string): DebugSession {
	...
	if (!Array.isArray(raw.eventLog))
		throw new Error("Invalid session: eventLog must be an array");
	...
	if (raw.mutationLog.length > MAX_ENTRIES)
		raw.mutationLog = raw.mutationLog.slice(-MAX_ENTRIES);
	if (raw.warningLog.length > MAX_ENTRIES)
		raw.warningLog = raw.warningLog.slice(-MAX_ENTRIES);
	if (raw.eventLog.length > MAX_ENTRIES)
		raw.eventLog = raw.eventLog.slice(-MAX_ENTRIES);
	if (raw.syncReadLog.length > MAX_ENTRIES)
		raw.syncReadLog = raw.syncReadLog.slice(-MAX_ENTRIES);
