...
export default function runVM(self, thread) {
  ...
  function sendMessages(...args: any[]) {
    args.forEach(items => {
  ...
  function stupidActionLoop(actionLoop) {
    actionsList = actionsList.concat(
      actionsPool.splice(0, actionsPool.length).sort(prioritySort)
    );
  ...
  function customRemoveChild(data) {
    var node = getNode(data.id, data);
    var child = getNode(data.childrenId, data);
    if (node && child) {
  ...
  function removeClass(data) {
    return getNode(data.id, data).classList.remove(data.class);
  }
  ...
  function addClass(data) {
    return getNode(data.id, data).classList.add(data.class);
  }
  // DOM action getStyleValue
  function getStyleValue(data) {
    return getNode(data.id, data).style[data.style];
  }
  ...
  function customInsertBefore(data) {
    ...
    var root = getNode(data.id, data);
    if (root) {
      var newEl = getNode(data.newId, data);
      if (newEl) {
        if (data.refId) {
          var refEl = getNode(data.refId, data);
          if (refEl) {
  ...
  function setParentNode(data) {
    var node = getNode(data.id, data);
    var parent = getNode(data.parent, data);
    node.parent = parent;
  }
